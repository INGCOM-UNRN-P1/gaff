---
title: "Guía de Extensión y Creación de Plugins: gaff"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de gaff"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-gaff-plugins)=
# Guía de Extensión y Plugins: gaff

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`gaff`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-gaff-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`gaff` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para gaff`.
- **Punto de Entrada Oficial**: `gaff.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-gaff-plugins-tutorial)=
...
Para que `gaff` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."gaff.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `gaff` la reconozca:
...
(manual-gaff-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `gaff` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = gaff.ejecutar_analisis(
...
(manual-gaff-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `gaff` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar gaff
        run: pip install -e ./gaff
        
      - name: Ejecutar Auditoría
        run: gaff check src/ include/ --json > reporte.json
````

---

(manual-gaff-plugins-ejercicios)=
